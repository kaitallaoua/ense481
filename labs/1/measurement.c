...
uint16_t raw_div_32_int(volatile int32_t a, volatile int32_t b) {
			...
			if (b != 0) {
			volatile int32_t c = a / b; // store it to prevent optmizations from removing it, there must be a better way
			}
...
uint16_t raw_div_64_int(volatile int64_t a, volatile int64_t b) {
			...
			if (b != 0) {
			volatile int64_t c = a / b; // store it to prevent optmizations from removing it, there must be a better way
			}
...
uint8_t get_pseudo_rand_uint8(void) {
	...
	// good enough for this game, since the entropy will come from the "random" times that people complete the tasks in.
	// make to use least significant bits as possible since they vary the most
...
uint32_t get_pseudo_rand_uint32(void) {
	...
	uint32_t rand32 = (rand8 << 24u) | (rand8 << 16u) | (rand8 << 8u) | (rand8 << 0u);
	return rand32;
